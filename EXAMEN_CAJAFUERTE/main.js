...
function resaltarPalabra() {
    ...
    for (let i = 0; i < menuItems.length; i++) {
        ...
        if (contenidoItem.includes(palabraBuscada) && palabraBuscada !== "") {
...
registro.addEventListener('click', function () {
    ...
    const horarioAperturaValor = horarioApertura.value;
    

    // Divide las cadenas en horas y minutos
    const [horasApertura, minutosApertura] = horarioAperturaValor.split(':');
    ...
    // Calcula la diferencia entre el horario de apertura y la hora actual
    const diferenciaHoras = horasAperturaEntero - horaActual;
    const diferenciaMinutos = minutosAperturaEntero - minutosActuales;

    const resultado = document.querySelector('#resultado');
    resultado.innerHTML=`Para abrir quedan: ${diferenciaHoras}horas y ${diferenciaMinutos} minutos`
